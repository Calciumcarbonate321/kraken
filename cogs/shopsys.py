...
class shop(commands.Cog):
    '''This  cog has the shop system of bot economy'''
    ...
    async def update_bank(self,user,change = 0,mode = "wallet"):
     ...
     return [users[str(user.id)]["wallet"]]
    ...
    async def open_account(self,user):
     ...
     if str(user.id) in users:
      ...
      users[str(user.id)] = {"wallet": 0, "bal": 0}
     with open("data/bank.json","w") as f:
        f.write(json.dumps(users,indent=4))
     return True
    ...
    async def open_shopacc(self,user):
     ...
     if str(user.id) in users:
      ...
      users[str(user.id)] = {"Inventory": [], "bag": []}
    ...
    async def inv(self,ctx,user : discord.User=None):
     ...
     try:
       ...
       invt = users[str(user.id)]["Inventory"]
     except:
       bag = []
       invt = []
     ...
     for eqp in invt:
       ...
       eqpname = eqp["eqname"]
       eqic = eqp["eqicon"]
       if equipd != "Nothing": #here ive used "Nothing" to imply whether there are no items in inv, see the comments in equipsys
         em.add_field(name = f"*Currently equipped : {eqpname}* *{eqic}*",value = "⠀")
         em.add_field(name = "⠀",value = "⠀")
         em.add_field(name = "⠀",value = "⠀")
       break
     for item in bag:
